...
class ForgotPasswordAction extends Action
{
  ...
  public function run(DataTransporter $data): array
  {
    ...
    $user = Apiato::call('User@FindUserByEmailTask', [$data->email, $data->field]);

    if ($data->field == "phone_login" && $user->email == null) {
      $user->email = "no-email-" . $user->id . "@toobibs.com";
      Apiato::call('User@UpdateUserTask', [["email" => $user->email], $user->id]);
    }
    // generate token
