...
```
api.toobib.site/endpoint?limit=100
```
...
```
api.toobib.site/endpoint?limit=100&page=2
```
...
```
api.toobib.site/endpoint?orderBy=created_at
```
...
```
api.toobib.site/endpoint?orderBy=name&sortedBy=desc
```
...
```
api.toobib.site/endpoint?search=keyword here
```
...
```
api.toobib.site/endpoint?search=first keyword;second keyword
```

#### Search in specific field:
```
api.toobib.site/endpoint?search=field:keyword here
```

#### Search in specific fields for multiple keywords: 
```
api.toobib.site/endpoint?search=field1:first field keyword;field2:second field keyword
```

#### Define query condition:

```
api.toobib.site/endpoint?search=field:keyword&searchFields=name:like
```
...
```
api.toobib.site/endpoint?search=field1:first keyword;field2:second keyword&searchFields=field1:like;field2:=;
```
...
```
api.toobib.site/endpoint?filter=id;status
```
...
```
api.toobib.site/endpoint?page=200
```
...
```
api.toobib.site/endpoint?include=relationship
```
...
```
api.toobib.site/endpoint?skipCache=true
```
...
```shell
curl -X POST -H "Accept: application/json" -H "Content-Type: application/x-www-form-urlencoded; -F "email=admin@admin.com" -F "password=admin" -F "=" "http://api.toobib.site/v2/register"
```

Calling protected endpoint (passing Bearer Token) example:

```shell
curl -X GET -H "Accept: application/json" -H "Authorization: Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..." -H "http://api.toobib.site/v1/users"
```
